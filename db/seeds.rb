...
boris = User.new(avatar: "https://media-exp1.licdn.com/dms/image/C4E03AQEfSGDe5SR0vQ/profile-displayphoto-shrink_400_400/0/1525092669872?e=1658966400&v=beta&t=cBOS9XthWw6hOOGnju9aBHsr_RqWCH0CNCcjMX63cBQ", name: "Boris Paillard", email:"boris@boris.com", password: "123456")
boris.save
  ...
  mentor = Mentor.new(title: "Batch Manager of LeWagon Madrid", expertise: "tech", description: "Fullstack bootcamp graduate from LeWagon, excixted to see where the world of tech will get me one line of code at a time. Skilled in Client Focus, Photography, Translation, Foreign Languages, and Live Video Streaming. Active personality and logical thinker, always looking for new challenges.", price: 10)
  mentor.user = gonzo
  mentor.save
...
10.times do
  ...
  mentor = Mentor.new(title: "Finance Manager at HSBC", expertise: "finance", description: "I am Germany's best-known hedge fund manager.
    ...
    His positive, absolute, relative and tested performance in the stock market crashes 1987, 2002 as well as in the corrections 1990 and 1994 is unique in Europe. His successful bearish speculations at Bremen Vulkan, MLP and WCM are well documented.", price: 50)
  ...
  mentor = Mentor.new(title: "Chairman at EY", expertise: "consulting", description: "I am the Global Chairman and CEO for EY, one of the largest professional services organizations in the world, which in the last fiscal year achieved US$40 billion in revenues and today has over 310,000 people serving clients in more than 150 countries, as well as one million alumni. Prior to being elected to my current post, I served as EY Global Managing Partner – Client Service, leading the execution of EY’s global strategy and its four geographical areas and four service lines.
    ...
    On the link below you can discover more about my career and current roles.", price: 800)
  mentor.user = User.where.not(id: gonzo.id)[4]
  mentor.save

  mentor = Mentor.new(title: "Junior Consultant at EY", expertise: "consulting", description: "I am a Junior Consultant at EY since 2011, I work with clients on a project or program related to their specialization.", price: 50)
  ...
  mentor = Mentor.new(title: "Fullstack Developer at Microsoft", expertise: "tech", description: "I handle all the work of databases, servers, systems engineering, and clients.", price: 740)
  mentor.user = User.where.not(id: gonzo.id)[7]
  mentor.save

  mentor = Mentor.new(title: "Blockchain Engeneer at Meta", expertise: "tech", description: "I perform the operations, designing, development, analyzing, implementing, and supporting a distributed blockchain network", price: 50)
  mentor.user = User.where.not(id: gonzo.id)[8]
  mentor.save

  mentor = Mentor.new(title: "Analyst at BlackRock", expertise: "finance", description: "I gather, interpret, and use complex data to develop actionable steps that will improve processes and optimize results", price: 50)
  mentor.user = User.where.not(id: gonzo.id)[9]
  mentor.save


  benedikt = User.new(avatar: "https://media-exp1.licdn.com/dms/image/C4D03AQHPEb156ucz8g/profile-displayphoto-shrink_800_800/0/1649668865376?e=1658966400&v=beta&t=doP_ljEIM-mWCh5dNSYlLQ_BaAxewRdilqW4GR-xVm4", name: "benedikt", email:"demotest@875.com", password: "123456")
  benedikt.save
